...
func main() {
	...
	tcp := listeners.NewTCP("t1", ":1884", nil)
   ...
   err = server.AddHook(new(auth.Hook), &auth.Options{
	...
	// `server.Publish` method. Subscribe to `direct/publish/smartwind` using your
	...
	go func() {
		...
		for range time.Tick(time.Second * 1) {
			err := server.InjectPacket(cl, packets.Packet{
				...
				TopicName: "direct/publish/smartwind",
				Payload:   []byte("injected scheduled message"),
			})
			if err != nil {
				server.Log.Error().Err(err).Msg("server.InjectPacket")
			}
			server.Log.Info().Msgf("main.go injected packet to direct/publish/smartwind")
		}
	}()
	...
	go func() {
		for range time.Tick(time.Second * 5) {
			//err := server.Publish("direct/publish/smartwind", []byte("packet scheduled message"), false, 0)
			if err != nil {
				server.Log.Error().Err(err).Msg("server.Publish")
			}
			server.Log.Info().Msgf("main.go issued direct message to direct/publish/smartwind")
		}
	}()
